...
parser.add_argument('--arch', type=str, default=' ', help='type of LR scheduler')
...
def main():
    ...
    target_model.arch_normal = tmp[3][0]
    target_model.arch_reduce = tmp[3][1]
    ...
    surrogate_model.arch_normal = tmp[4][0]
    surrogate_model.arch_reduce = tmp[4][1]
    ...
    target_nparam = utils.compute_nparam(target_model, size_,target_model.arch_normal ,target_model.arch_reduce, 'null')
    surrogate_nparam = utils.compute_nparam(surrogate_model, size_,surrogate_model.arch_normal, surrogate_model.arch_reduce, 'null')
    ...
    logging.info('target model: nparam {} '.format(target_nparam))
    logging.info('surrogate model: nparam {} '.format(surrogate_nparam))
    ...
    save_dict['target_nparam'] = target_nparam
    # save_dict['target_run_time'] = target_run_time
    save_dict['surrogate_nparam'] = surrogate_nparam
