...
parser.add_argument('--rt', type=str, default='absolute', help='reward_type')
...
tmp = 'train_search/train_search_twin'
if args.debug:
    tmp = os.path.join(tmp, 'debug')
if args.imitation:
    tmp = os.path.join(tmp, 'imitation')
...
def main():
    ...
    for iteration in range(args.its):
        ...
        logging.info('iteration %d lr %e', iteration, args.lr)
        ...
        for step, (input, target) in enumerate(test_queue):
            ...
            reward, optimized_acc_adv, acc_adv, n_ent, r_ent, loss = transformer.step(input, target)
            if step == args.accu_batch - 1:
                summaryWriter.add_scalar('reward', reward, iteration)
                summaryWriter.add_scalar('loss', loss, iteration)
                logging.info('Updating Theta iteration=%d optimized_acc_adv=%.2f acc_adv=%.2f reward=%.2f, normal_ent = %.2f, reduce_ent=%.2f,loss=%.3f', iteration, optimized_acc_adv, acc_adv, reward, n_ent, r_ent, loss)
