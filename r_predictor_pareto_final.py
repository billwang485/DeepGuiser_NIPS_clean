...
parser.add_argument('--pareto_dir', type=str, default=' ', help='The path to best archs')#
parser.add_argument('--name', type=str, default='ResBlock', help='The path to best archs')#
...
args.save = os.path.join(args.prefix, 'rebuttal/predictor_final', args.save)
args.cutout = False
...
def main():
    ...
    assert os.path.exists(args.pareto_dir)

    arch_list_list = torch.load(os.path.join(args.pareto_dir, 'result_list_list'), map_location='cpu')
    logging.info(args.pareto_dir)

    for interval in range(len(arch_list_list)):
        tmp_acc_adv = 0
        chosen_arch = None
        for candi in arch_list_list[interval]:
            if tmp_acc_adv < candi['acc_adv']:
                chosen_arch = candi
        
        surrogate_arch = chosen_arch['surrogate_arch']
        target_arch = chosen_arch['target_arch']
        ...
        utils.save(surrogate_model, os.path.join(args.save, 'surrogate_model_{}.pt'.format(interval)))
        ...
        for step, (input, target) in enumerate(test_queue):
            ...
            if step % args.report_freq == 0:
                logging.info('Step=%03d: Surrogate model %d surrogate_acc_clean=%.2f surrogate_acc_adv=%.2f target_acc_clean_baseline=%.2f adv_acc_baseline=%.2f target_acc_clean=%.2f target_acc_adv=%.2f reward=%.2f',\
                    step, interval, surrogate_acc_clean.avg, surrogate_acc_adv.avg, target_acc_clean_baseline.avg, adv_acc_baseline.avg, target_acc_clean.avg, target_acc_adv.avg, reward.avg )
        ...
        #             interval, surrogate_acc_clean.avg, surrogate_acc_adv.avg, target_acc_clean_baseline.avg, adv_acc_baseline.avg, target_acc_clean.avg, target_acc_adv.avg, reward.avg )
        logging.info('adv_acc_baseline=%.2f budget=%.5f', adv_acc_baseline.avg, chosen_arch['budget'])
        save_dict = {}
        ...
        save_dict['budget'] = chosen_arch['budget']    
        torch.save(save_dict, os.path.join(args.save, 'surrogate_{}_result'.format(interval)))
