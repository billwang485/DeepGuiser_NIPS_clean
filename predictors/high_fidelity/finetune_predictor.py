...
parser.add_argument('--train_data_path', type=str, default='final_train_data515', help=' ')
parser.add_argument('--test_data_path', type=str, default='final_test_data515', help=' ')
...
def main():
    ...
    if args.load_dataset == False:
        train_data = PredictorDataSet(None, \
            # 'predictor/hf_data/train_data_enhanced',
            args.train_data_path,\
            # 'predictor/4099/train_data_free_train', \
            # 'no_name/train_data_free_train', \
            args)
        if not args.debug:
    ...
    train_queue = torch.utils.data.DataLoader(
        ...
        # sampler=predictor_sampler(indices[:split]),
    ...
    # train_queue = torch.load('predictor/pretrain/Jul 31 18 59 17/train_queue')
    # valid_queue = torch.load('predictor/pretrain/Jul 31 18 59 17/valid_queue')
    # else:
    #     train_queue = torch.utils.data.DataLoader(
    #         train_data, args.batch_size,
    #         sampler=predictor_sampler(indices[:split]),
    ...
    #         valid_queue = torch.utils.data.DataLoader(
    #             train_data, args.batch_size,
    #             sampler=torch.utils.data.sampler.predictor_sampler(indices[split:num_train]),
    #             pin_memory=True, num_workers=2
    #         )
    #     else:
    #         test_data = PredictorDataSet(None, \
    #             os.path.join(args.data_dir, 'train_data_free_'), \
    #             args)
    #         torch.save(test_data, os.path.join(args.data_dir, 'test_data_debug'))
    #         valid_queue = torch.utils.data.DataLoader(
    #             test_data, args.batch_size,
    #             sampler=torch.utils.data.sampler.predictor_sampler(list(range(len(test_data)))),
    #             pin_memory=True, num_workers=2
    #         )
    ...
    if args.pw != ' ':
        ...
        tmp = torch.load(args.pw, map_location='cpu')

        model.predictor.load_state_dict(tmp, strict = False)
        # model.predictor.arch_embedder_1.load_state_dict(model.predictor.arch_embedder.state_dict())
