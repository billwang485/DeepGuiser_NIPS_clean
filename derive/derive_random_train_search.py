...
parser.add_argument('--thre', type=int, default=30, help='hidden dimension')
...
def main():
    ...
    for i in range(20):
        arch_normal = tmp[i][0]['target_arch'][0]
        arch_reduce = tmp[i][0]['target_arch'][1]
        ...
        result = model.derive_optimized_arch(arch_normal, arch_reduce, 1, logger, args.save, "derive", normal_concat=genotype.normal_concat, reduce_concat=genotype.reduce_concat)
        ...
        torch.save(result, os.path.join(args.save, 'result_dict_{}_{}'.format(args.arch, i)))
