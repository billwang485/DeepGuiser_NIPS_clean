...
# parser.add_argument('--thre', type=int, default=8, help='reward_type')
parser.add_argument('--n_archs', type=int, default=10, help='random seed')
...
def main():
    ...
    for i in range(100):
        ...
        arch_reduce = model.arch_normal_master_demo.forward()
        ...
        if args.store == 1:
            utils.save(model.arch_transformer, os.path.join(args.save, 'model_final_{}.pt'.format(i)))
            torch.save(model.best_pair, os.path.join(args.save, "best_transform_pair_{}".format(i)))
        
        genotype = eval("genotypes.%s" % args.arch)
        # arch_normal, arch_reduce = utils.genotype_to_arch(genotype, args.op_type)
        
        result_ = model.derive_optimized_arch(arch_normal, arch_reduce, 1, logger, args.save, "derive", normal_concat=genotype.normal_concat, reduce_concat=genotype.reduce_concat)
        ...
        for i, ((op, _, _), (op1, _, _)) in enumerate(zip(target_arch_list[-1][1], surrogate_arch_list[-1][1])):
            sample_arch_count[op][op1] = sample_arch_count[op][op1] + 1
        train_info = (0, 0, result_["absolute_predictor_reward"], result_["target_arch"], result_["surrogate_arch"])
        torch.save(result_, os.path.join(args.save, 'result_dict_{}_{}'.format(args.arch, i)))
        torch.save(train_info, os.path.join(args.save, 'train_info_{}_{}'.format(args.arch, i)))
