...
def main():
    ...
    utils.load(model, os.path.join(args.pw, 'model_final.pt')) 
    ...
    result = model.derive_optimized_arch(model_twin, derive_queue, arch_normal, arch_reduce, 10, logger, args.save, "derive", normal_concat=genotype.normal_concat, reduce_concat=genotype.reduce_concat)

    torch.save(result, os.path.join(args.save, 'result_dict'))
    train_info = (0, 0, result["absolute_supernet_reward"], result["target_arch"], result["surrogate_arch"])
    torch.save(result, os.path.join(args.save, 'result_dict'))
