...
parser.add_argument('--pw', type=str, default='pretrain_models/LOOSE', help=' ')
...
def main():
    ...
    _, valid_queue, _ = utils.get_train_queue(args)
    model = Network( \
    ...
    tmp = torch.load('train_data_constrain_VGG', map_location='cpu')
    ...
    for data in tmp:
        ...
        for step, (input, target) in enumerate(valid_queue):
            if args.accu_batch <= step:
                break
            n = input.size(0)
            input = input.to(device)
            target = target.to(device)
            reward_, acc_clean_, optimized_acc_clean_, acc_adv_, optimized_acc_adv_ = model.evaluate_transfer(model_twin, (arch_normal, arch_reduce), (optimized_normal, optimized_reduce), input, target)
            # logging.info("number: %d step: %d", iteration, step)
            reward.update(reward_.item(), n)
    ...
    for data in tmp:
        ...
        score0 = model.predictor.forward([arch_normal_, arch_reduce_], [optimized_normal_, optimized_reduce_]).item()
        score1 = model.predictor.forward([arch_normal_, arch_reduce_], [arch_normal_, arch_reduce_]).item()
        score_1_list.append(score0)
        label_list.append(data['absolute_reward'])
    patk = utils.patk(label_list, score_1_list, 4)

    logging.info('predictor result')

    kendall = kendalltau(score_1_list, label_list).correlation
    logging.info('score_list=')
    logging.info(score_1_list)
    logging.info('label_list=')
    logging.info(label_list)
    logging.info(patk)
    logging.info(kendall)
    ...
    score_list_pos = deepcopy(score_list)
    score_list_pos.sort(reverse=False)
    real_score_0 = []
    score_1_list_pos = deepcopy(score_1_list)
    score_1_list_pos.sort(reverse=False)
    real_score_1 = []
    for i in range(20):
        real_score_0.append(score_list_pos.index(score_list[i]))
        real_score_1.append(score_1_list_pos.index(score_1_list[i]))
    # real_score = real_score_0 + real_score_1
    real_score = [0] * 20
    for i in range(20):
        real_score[i] = real_score_0[i] + real_score_1[i]

    patk = utils.patk(label_list, real_score, 4)

    logging.info('final result')

    kendall = kendalltau(real_score, label_list).correlation
    logging.info('score_list=')
    logging.info(real_score)
    logging.info('label_list=')
    logging.info(label_list)
    logging.info(patk)
    logging.info(kendall)
