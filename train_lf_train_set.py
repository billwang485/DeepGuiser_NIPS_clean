...
def main():
    ...
    for iteration in range(args.num):
        ...
        logging.info('number %d', iteration)

        target_normal = model.arch_normal_master.forward()
        target_reduce = model.arch_reduce_master.forward()

        if not args.constrain:
            surrogate_normal = model.arch_normal_master.forward()
            surrogate_reduce = model.arch_reduce_master.forward()
        else:
            surrogate_normal = model.uni_random_transform(target_normal)
            surrogate_reduce = model.uni_random_transform(target_reduce)
        ...
        assert iteration == len(train_dataset)
