...
args.save = os.path.join(args.prefix, 'z/transformer_random_test', args.save)
args.cutout = False
...
def main():
    ...
    utils.save(surrogate_model, os.path.join(args.save, 'surrogate_model_predictor.pt'))
    ...
    torch.save(save_dict, os.path.join(args.save, 'surrogate_predictor_result'.format(num)))
    ...
    file_merger.write(os.path.join(args.save, "disguised_target_predictor.pdf"))
    ...
    random_reward_mean = utils.AvgrageMeter()
    for i in range(4):
        surrogate_model = FinalNet(
        args.init_channels, CIFAR_CLASSES, args.layers, criterion, device, steps=args.num_nodes, controller_hid=args.controller_hid, entropy_coeff=args.entropy_coeff, edge_hid = args.edge_hid, transformer_nfeat = args.transformer_nfeat, transformer_nhid = args.transformer_nhid, transformer_dropout = args.transformer_dropout, transformer_normalize = args.transformer_normalize, loose_end = args.loose_end, op_type=args.op_type
    )
        ...
        target_acc_adv = utils.AvgrageMeter()
        reward = utils.AvgrageMeter()
        utils.save(surrogate_model, os.path.join(args.save, 'surrogate_random_{}.pt'.format(i)))
        ...
        save_dict['adv_acc_baseline'] = adv_acc_baseline.avg
        save_dict['reward'] = reward.avg
        torch.save(save_dict, os.path.join(args.save, 'random_surrogate_{}_result'.format(i)))
        random_reward_mean.update(reward.avg, 1)
        ...
        file_merger.write(os.path.join(args.save, "random_target_{}.pdf".format(i)))
        ...
        os.remove(os.path.join(args.save, "disguised_reduce.pdf"))
        os.remove(os.path.join(args.save, "disguised_reduce"))
    torch.save(random_reward_mean.avg, os.path.join(args.save, 'random_reward_mean'))
    logging.info(random_reward_mean.avg)
