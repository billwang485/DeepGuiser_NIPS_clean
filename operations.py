...
OPS = {
    ...
    'bottleneck_1x1': lambda C,stride, affine: BottleNeck(C, C, 1, stride, 0, affine=affine),
    'bottleneck_3x3': lambda C,stride, affine: BottleNeck(C, C, 3, stride, 1, affine=affine),
    'bottleneck_5x5': lambda C,stride, affine: BottleNeck(C, C, 5, stride, 2, affine=affine),
}
...
class BottleNeck(nn.Module):
    def __init__(self, C_in, C_out, kernel_size, stride, padding, affine=True):
        ...
        nn.Conv2d(C_in, c//4, kernel_size=kernel_size, stride=stride, padding=padding),
        nn.BatchNorm2d(c//4, affine=affine),
        nn.ReLU(inplace=True),
        nn.Conv2d(c//4, C_out, kernel_size=kernel_size, stride=1, padding=padding),
        nn.BatchNorm2d(C_out, affine=affine),
        nn.ReLU(inplace=True)
        )
        self.branch = ReLUConvBN(C_in, C_out, kernel_size=kernel_size, stride=stride, padding=padding)

    def forward(self, x):
        return self.op(x) + self.branch(x)
